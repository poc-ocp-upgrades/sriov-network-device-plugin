...
var _ = Describe("Device Plugin APIs", func() {
	...
	BeforeEach(func() {
		...
		sm = &sriovManager{devices: make(map[string]pluginapi.Device), socketFile: fmt.Sprintf("%s.sock", pluginEndpointPrefix), termSignal: make(chan bool, 1), stopWatcher: make(chan bool)}
	...
	It("Check Allocate success", func() {
		...
		request = &pluginapi.AllocateRequest{ContainerRequests: []*pluginapi.ContainerAllocateRequest{{DevicesIDs: devices}}}
	...
	It("Check Allocate with non-existing device", func() {
		request := new(pluginapi.AllocateRequest)
		response := new(pluginapi.AllocateResponse)
		request = &pluginapi.AllocateRequest{ContainerRequests: []*pluginapi.ContainerAllocateRequest{{DevicesIDs: []string{"0000:00:02.1"}}}}
		response, err = sm.Allocate(ctx, request)
		Expect(response).To(BeNil())
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("Invalid allocation request with non-existing device 0000:00:02.1"))
	})
	It("Check Allocate with unhealthy device", func() {
		...
		Expect(response).To(BeNil())
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("Invalid allocation request with unhealthy device 0000:00:02.0"))
	})
	...
	It("Check ListAndWatch send device states fail", func() {
		...
		err = sm.ListAndWatch(empty, stream)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("Error. Cannot update device states"))
	})
})
